  ...
  <li>
    <Link
      ...
      className={cn(
        ...
        current ? "bg-gray-50" : "hover:bg-gray-100",
      )}
    >
      {icon}
...
export function LogoutButton() {
  return (
    <form action={logout}>
      <button className="flex w-full items-center gap-2 rounded-lg px-2 py-1 transition hover:bg-gray-100">
        <LogoutIcon />
...
export default function Navigation({
  ...
  const [navList, setNavList] = useState(
    isAdmin ? teacherNavList : studentNavList,
  );
  ...
  return (
    <div className="flex h-screen">
      <div className="w-52 shadow">
        <div className="flex h-full flex-col p-3 pt-0 text-lg">
          <p className="mx-2 my-8">手軽にアルゴル</p>

          <ul className="flex h-full flex-col gap-2 text-gray-900 [&>*:last-child]:mt-auto">
      ...
      <div className="flex-1 overflow-auto">
        <div className={cn("min-h-full", padding && "p-8")}>{children}</div>
      </div>
    </div>
  );
}
